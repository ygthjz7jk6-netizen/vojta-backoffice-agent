...
- 🔴 Nastavit `.env.local` s klíči

---

## Fáze 1 — Databáze & základní backend

- 🔴 Napsat Supabase migrace (`001_initial.sql`) — všechny tabulky
- 🔴 Aktivovat pgvector extension v Supabase
...
- 🔴 `get_calendar_slots` — Google Calendar API
- 🔴 `draft_communication` — Gmail draft s approval flow
- 🔴 `create_visualization` — Chart.js grafy (PNG nebo embed)
- 🔴 `generate_report` — markdown report, volitelně PPTX
- 🔴 `schedule_action` — zápis do `scheduled_tasks` s approval
...
- 🔴 Sources panel — zobrazení citací vedle odpovědi
- 🔴 Quick Actions tlačítka (Shrnutí týdne, Noví klienti Q1, atd.)
